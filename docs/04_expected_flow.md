...
2. Access Token 만료 시 정상 처리 흐름 정의
3. 동시 인증 실패 상황에서 Refresh 요청 단일화 기준 정의
4. 실패 요청 대기 및 재시도 기준 정의
5. 성능시험 전 확인해야 할 인증 흐름 기준 제공
```
...
Refresh Token은 Access Token이 만료되었을 때 새 Access Token을 발급받기 위한 값입니다.
...
2. Authorization Header에 Access Token을 포함한다.
3. 서버가 Access Token의 서명, 만료시간, 토큰 타입을 검증한다.
4. 검증 성공 시 요청을 처리한다.
5. 서버가 정상 응답을 반환한다.
```
...
## 5. Access Token 만료 시 정상 흐름

Access Token이 만료된 경우 정상 흐름은 다음과 같습니다.

```text
1. 클라이언트가 만료된 Access Token으로 API를 요청한다.
2. 서버가 Access Token 만료를 감지한다.
3. 서버는 401 Unauthorized 또는 사전에 정의된 인증 실패 응답을 반환한다.
4. 클라이언트는 Access Token 만료 응답을 감지한다.
...
문제는 Access Token이 만료된 상태에서 여러 요청이 동시에 인증 실패했을 때 발생합니다.

---

## 7. 동시 인증 실패 시 정상 Refresh 처리

Access Token 만료 상태에서 API 요청 8개가 동시에 실패한 경우, 정상 구조는 다음과 같습니다.

```text
1. API 요청 8개가 동시에 발생한다.
2. Access Token 만료로 8개 요청이 인증 실패한다.
3. 최초 1개 요청만 Refresh 요청을 시작한다.
4. 나머지 실패 요청은 Refresh 완료까지 대기한다.
...
## 8. 비정상 흐름 예시 1: Refresh 요청 중복 발생
...
## 9. 비정상 흐름 예시 2: Refresh는 1회이나 일부 요청만 복구
...
1. 401 응답 감지
2. Refresh 진행 여부 확인
3. Refresh 미진행 상태면 Refresh 요청 시작
4. Refresh 진행 중이면 실패 요청을 대기열에 등록
5. Refresh 성공 시 새 Access Token 저장
6. 대기 중인 요청에 새 Access Token 전달
7. 실패했던 요청 재시도
8. Refresh 실패 시 대기 요청 전체 실패 처리
9. 토큰 정리 및 세션 만료 처리
10. 무한 재시도 방지
```
...
## 12. Access Token과 Refresh Token 타입 검증
...
2. API 요청 8개가 동시에 실패해도 Refresh 요청은 1회만 발생
3. Refresh 요청에는 Refresh Token 또는 세션 식별값이 사용됨
4. Access Token만으로 Refresh가 수행되지 않음
5. Refresh 성공 후 기존 실패 요청들이 재시도됨
6. 최종적으로 API 요청 8개가 모두 성공함
7. Refresh 실패 시 전체 요청이 정리되고 세션 만료 처리됨
```
...
Access Token 만료 시 Refresh Token으로만 재발급
동시 인증 실패 시 Refresh 요청은 1회만 수행
나머지 실패 요청은 대기 후 재시도
Refresh 실패 시 전체 세션 정리
```

따라서 본 이슈의 핵심은 API 요청이 여러 개 발생하는지 여부가 아니라, Access Token 만료 시 여러 인증 실패 요청이 어떻게 복구되는지입니다.
